...
// CONTROLEUR DE GESTION DES PARTIES ET DU GAMING
...
const getBatch = async (req, res, next) => {
  try {
    ...
    // 2. Verrouillage preventif atomique immediat dans l'historique 30 jours
...
const validateSession = async (req, res, next) => {
  try {
    const result = await gameService.validateFinalSession(req.user._id, req.body);
    res.status(200).json({ status: 'success', data: result });
  } catch (error) {
    next(error);
  }
